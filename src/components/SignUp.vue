  ...
  <b-row>
    <b-col>
      ...
      <b-form v-if="signupState = 'open'" @submit.prevent="onSubmit" @reset="clearForm">
        ...
        <!-- Child Info -->
        <b-card bg-variant="light" class="mb-3" v-for="childNum in childCount" :key="childNum"
          v-if="drawComponents">

          <child-info v-model="form.children[childNum - 1]" :child-count="childCount" :child-num="childNum"
            @destroy="removeChild(childNum)" :openSessions="openSessions">
          </child-info>
        </b-card>
        ...
        <b-alert variant="danger" dismissible @dismissed="error = null" fade :show="!!error">
          <h4>Sorry</h4>
          <p>There was a problem submitting your sign-up form. Please use the contact
            form if you need help. Thanks.</p>
